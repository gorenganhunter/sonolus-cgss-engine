...
import { easeInBack } from "../../../../../../shared/src/engine/data/utils"
...
export class Decorator extends Archetype {
    ...
    updateParallel() {
        if (this.import.visible >= 0) {
            if (time.scaled > this.visualTime.max + note.duration * 0.2) this.despawn = true
            if (time.scaled < this.visualTime.min + this.import.visible / 100 * note.duration) return
        } else {
            if (time.scaled > this.visualTime.min - this.import.visible / 100 * note.duration) this.despawn = true
        }
    ...
    drawTop() {
        ...
        skin.sprites.draw(this.sprite, new Rect({ l: this.x - w, r: this.x + w, t: this.y - h, b: this.y }), this.note.z, 1)
    }
}
