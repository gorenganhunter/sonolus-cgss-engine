...
export class DecoratorConnector extends BaseConnector {
    ...
    despawnTime(): number {
        return this.tailImport.visible >= 0 ? this.tail.scaledTime + note.duration : this.tail.scaledTime - note.duration * (1 - (this.tailImport.visible - 10) / -100)
    }
    ...
    renderConnector() {
        ...
        const t1 = getNoteTransformedTime(this.head.spawnTime, this.head.scaledTime, /*this.headImport.visible < 0 ? Math.min(time.scaled, this.head.scaledTime - note.duration * (1 + this.headImport.visible / 100)) : */time.scaled, true, false)
        const t2 = getNoteTransformedTime(this.tail.spawnTime, this.tail.scaledTime, /*this.tailImport.visible >= 0 ? Math.max(time.scaled, this.tail.spawnTime + note.duration * this.tailImport.visible / 100) : */time.scaled, true, false)
        const tmid = (t1 + t2) / 2
        ...
        for (let i = 0; i < div; i++) {
            ...
            const size = {
                min: Math.remap(t1, t2, this.headImport.size / 100, this.tailImport.size / 100, t.min),
                max: Math.remap(t1, t2, this.headImport.size / 100, this.tailImport.size / 100, t.max),
            }
    ...
    //
    //     const visibleTime = {
    //         min: this.headImport.visible >= 0 ? this.head.scaledTime : Math.max(time.scaled + note.duration * (this.headImport.visible < 0 ? 1 - this.headImport.visible / -100 : 0), this.head.scaledTime/* - note.duration * (this.headImport.visible >= 0 ? 0 : 1 + this.headImport.visible / 100)/*, time.scaled - note.duration * (this.headImport.visible >= 0 ? -1 : 1 + this.headImport.visible / 100)*/),
    //         max: Math.min(this.tail.scaledTime, time.scaled + note.duration * (this.tailImport.visible < 0 ? 1 : 1 - this.tailImport.visible / 100))
    //     }
    //
    //     const t1 = getNoteTransformedTime(this.head.spawnTime, this.head.scaledTime, visibleTime.min)
    //     const t2 = getNoteTransformedTime(this.tail.spawnTime, this.tail.scaledTime, visibleTime.max)
    //     const tmid = (t1 + t2) / 2
    ...
    //     const xmid = getNoteXPosition(this.tailImport.spawnLane * 0.75, this.tailImport.lane, tmid)
    //
    //     const div = Math.ceil((t2 - t1) * options.connectorQuality)
    //
    //     let xS: any = {
    //         min: 0,
    //         max: 0
    ...
    //
    //         const size = {
    //             min: Math.remap(this.head.scaledTime, this.tail.scaledTime, this.headImport.size, this.tailImport.size, scaledTime.min) / 100,
    //             max: Math.remap(this.head.scaledTime, this.tail.scaledTime, this.headImport.size, this.tailImport.size, scaledTime.max) / 100,
    //         }
    //
    //         const t = {
    //             min: Math.lerp(t1, t2, i / div),
    ...
    //
    //         // const l = {
    //         //     min: this.getL(scaledTime.min, s.min * size.min),
    //         //     max: this.getL(scaledTime.max, s.max * size.max),
    //         // }
    //         //
    //         // const r = {
    //         //     min: this.getR(scaledTime.min, s.min * size.min),
    //         //     max: this.getR(scaledTime.max, s.max * size.max),
    //         // }
    //
    //         const layout = {
    //             x1: (x.min - xS.min)/*  * (y.min + yS) */,
    ...
    //             y4: (y.min - yS.min),
    //         }
    //         //debug.log(options.connectorAlpha)
    //         // skin.sprites.draw(this.sprite.connector, layout, this.connector.z, options.connectorAlpha, "lr", 64, { x: this.tailImport.spawnLane * 0.75 + 0.36 * (this.head.lane - this.tailImport.spawnLane * 0.75), y: 0 }, { x: this.head.lane + 0.66 * (this.head.lane - this.tailImport.spawnLane * 0.75), y: -0.56 })
    //         if (this.sprite.connector === this.sprite.connector2) {
    //             skin.sprites.draw(this.sprite.connector, layout, this.connector.z, 1)
    //         } else {
    //             skin.sprites.draw(this.sprite.connector, layout, this.connector.z, 1 - (i + 0.5) / div)
    //             skin.sprites.draw(this.sprite.connector2, layout, this.connector.z + 0.0000001, (i + 0.5) / div)
    //         }
    //
    //         if (i === 0 && scaledTime.min === this.head.scaledTime && !this.headImport.prevRef) {
    //             const w = 0.25 * s.min * this.headImport.size / 100
    //             const dx = layout.x4 - layout.x1
    //             const dy = layout.y4 - layout.y1
    //             const a = Math.atan2(dy / scaledScreen.wToH, dx)
    //
    ...
    //         }
    //
    //         if (i === div - 1 && scaledTime.max === this.tail.scaledTime && !this.tailImport.nextRef) {
    //             const w = 0.25 * s.max * this.tailImport.size / 100
    //             const dx = layout.x3 - layout.x2
    //             const dy = layout.y3 - layout.y2
    //             const a = Math.atan2(dy / scaledScreen.wToH, dx)
    //
